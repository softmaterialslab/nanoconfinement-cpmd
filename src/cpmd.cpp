...
void cpmd(vector<PARTICLE> &ion, INTERFACE &box, vector<THERMOSTAT> &real_bath, vector<THERMOSTAT> &fake_bath,
          vector<BIN> &bin, CONTROL &fmdremote, CONTROL &cpmdremote) {
...
//     // compute density profile
        if (num >= cpmdremote.hiteqm && (num % cpmdremote.freq == 0)) {
            ...
            compute_density_profile(num, density_profile_samples, mean_positiveion_density, mean_sq_positiveion_density,
                                    mean_negativeion_density, mean_sq_negativeion_density, ion, box, bin, cpmdremote);
        }
    ...
    if (world.rank() == 0) {
        for (unsigned int b = 0; b < positiveion_density_profile.size(); b++)
            list_p_profile << (-0.5 * box.lz + b * bin[b].width) * unitlength << setw(15)
                           << positiveion_density_profile.at(b)
                           << setw(15) << p_error_bar.at(b)
                           << endl; // change in the z coordinate, counted from leftwall

        for (unsigned int b = 0; b < negativeion_density_profile.size(); b++)
            list_n_profile << (-0.5 * box.lz + b * bin[b].width) * unitlength << setw(15)
                           << negativeion_density_profile.at(b)
                           << setw(15) << n_error_bar.at(b)
                           << endl; // change in the z coordinate, counted from leftwall
    }
