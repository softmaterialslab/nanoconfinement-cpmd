...
#pragma omp parallel for schedule(dynamic) default(shared) private(k, i, insum1, insum2, vec)
...
#pragma omp parallel for schedule(dynamic) default(shared) private(k, i, l, vec, dz, r_1, r_2, gcsh_z, gcsh_inf, \
...
#pragma omp parallel for schedule(dynamic) default(shared) private(k, j, i, vec, dz, r_1, r_2, factor, hcsh, E_z, \
  grad_G, grad_ndot_G, \
 ...
 + hqw1 + hqEw1_1 + hw1Eq_1 + hqw2 + hqEw2_2 + hw2Eq_2) ^ (scalefactor));
    ...
    // ion-ion
#pragma omp parallel for schedule(dynamic) default(shared) private(k, j, i, vec, r_1, \
