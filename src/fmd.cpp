...
void fmd(vector<PARTICLE> &ion, INTERFACE &box, CONTROL &fmdremote, CONTROL &cpmdremote) {
    ...
    for (unsigned int k = 0; k < box.leftplane.size(); k++) {
        box.leftplane[k].mu = fmdremote.fakemass * box.leftplane[k].a *
                              box.leftplane[k].a;                        // Assign mass to the fake degree
    ...
    for (unsigned int k = 0; k < box.rightplane.size(); k++) {
        box.rightplane[k].mu = fmdremote.fakemass * box.rightplane[k].a *
                               box.rightplane[k].a;                        // Assign mass to the fake degree
        // s[k].w = 0.0;								// Initialize fake degree value		(unconstrained)
...
*/
    ...
    for (unsigned int k = 0; k < box.leftplane.size(); k++)
        box.leftplane[k].w = box.leftplane[k].w - sigma_left / (box.leftplane[k].a *
                                                                (box.leftplane.size()));                // Force to satisfy constraint
    long double sigma_right = constraint_right(ion, box);
    for (unsigned int k = 0; k < box.rightplane.size(); k++)
        box.rightplane[k].w = box.rightplane[k].w - sigma_right / (box.rightplane[k].a *
                                                                   (box.rightplane.size()));                // Satisfy constraint

    long double sigmadot_left = dotconstraint_left(box);
    for (unsigned int k = 0; k < box.leftplane.size(); k++)
        box.leftplane[k].vw = box.leftplane[k].vw - sigmadot_left / (box.leftplane[k].a *
                                                                     (box.leftplane.size()));                // Force to atisfy time derivative of the constraint
    long double sigmadot_right = dotconstraint_right(box);
    for (unsigned int k = 0; k < box.rightplane.size(); k++)
        box.rightplane[k].vw = box.rightplane[k].vw - sigmadot_right / (box.rightplane[k].a *
                                                                        (box.rightplane.size()));                // Satisfy time derivative of the constraint
...
/*  ofstream fmdv, fmde, fmdtic;
  char data[200];
  sprintf(data, "outfiles_windows/fmdv_%.06d.dat", fmdremote.verify);
  fmdv.open(data);
  sprintf(data, "outfiles_windows/fmde_%.06d.dat", fmdremote.verify);
  fmde.open(data);
  sprintf(data, "outfiles_windows/fmdtic_%.06d.dat", fmdremote.verify);
  fmdtic.open(data);
*/
    //cout << "Time step " << fmdremote.timestep << endl;
