...
void compute_n_write_useful_data(int cpmdstep, vector<PARTICLE>& ion, vector<THERMOSTAT>& real_bath, vector<THERMOSTAT>& fake_bath, INTERFACE& box)
{
  ...
  list_temperature << cpmdstep << setw(15) << 2*particle_kinetic_energy(ion)/(real_bath[0].dof*kB) << setw(15) << real_bath[0].T << setw(15) << 2*fake_kinetic_energy(box)/(fake_bath[0].dof*kB) << setw(15) << fake_bath[0].T << endl;
...
double verify_with_FMD(int cpmdstep,  vector<PARTICLE>& ion, INTERFACE& box, CONTROL& fmdremote, CONTROL& cpmdremote)
{
  ...
  track_density_left << cpmdstep << setw(15) << box.leftplane[0].w << setw(15) << box_exact.leftplane[0].w << endl;
  track_density_right << cpmdstep << setw(15) << box.rightplane[0].w << setw(15) << box_exact.rightplane[0].w << endl;
  ...
  for (unsigned int k = 0; k < box_exact.leftplane.size(); k++)
    out_correct_ind_left << k+1 << setw(15) <<box_exact.leftplane[k].posvec << setw(15) << sqrt(box_exact.leftplane[k].posvec.x * box_exact.leftplane[k].posvec.x + box_exact.leftplane[k].posvec.y * box_exact.leftplane[k].posvec.y) << setw(15) << box_exact.leftplane[k].wmean << endl;
  ...
  for (unsigned int k = 0; k < box_exact.rightplane.size(); k++)
     out_correct_ind_right << k+1 << setw(15) <<box_exact.rightplane[k].posvec << setw(15) << sqrt(box_exact.rightplane[k].posvec.x * box_exact.rightplane[k].posvec.x + box_exact.rightplane[k].posvec.y * box_exact.rightplane[k].posvec.y) << setw(15) << box_exact.rightplane[k].wmean << endl;
  ...
  for (unsigned int k = 0; k < box.leftplane.size(); k++)
    out_cpmd_ind_left<< k+1 << setw(15) <<box.leftplane[k].posvec << setw(15) << sqrt(box.leftplane[k].posvec.x * box.leftplane[k].posvec.x + box.leftplane[k].posvec.y * box.leftplane[k].posvec.y) << setw(15) << box.leftplane[k].w << endl;
  ...
  for (unsigned int k = 0; k < box.rightplane.size(); k++)
     out_cpmd_ind_right<< k+1 << setw(15) <<box.rightplane[k].posvec << setw(15) << sqrt(box.rightplane[k].posvec.x * box.rightplane[k].posvec.x + box.rightplane[k].posvec.y * box.rightplane[k].posvec.y) << setw(15) << box.rightplane[k].w << endl;
