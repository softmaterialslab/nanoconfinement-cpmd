...
void cpmd(vector<PARTICLE>&, INTERFACE&, vector<THERMOSTAT>&, vector<THERMOSTAT>&, vector<BIN>&, CONTROL&, CONTROL&);
...
int main(int argc, char* argv[]) 
{
  ...
  desc.add_options()
      ...
      ("bx,X", value<double>(&bx)->default_value(22.848), "box length in x direction in nanometers")					// enter in nanometers
      ("by,Y", value<double>(&by)->default_value(22.848), "box length in y direction in nanometers")					// enter in nanometers
      ...
      ("saltion_diameter_in,d", value<double>(&saltion_diameter_in)->default_value(0.714), "salt ion diameter inside")		// enter in nanometers
      ("fraction_diameter,g", value<double>(&fraction_diameter)->default_value(1.4), "for interface discretization width")
      ...
      ("chain_length_fake,D", value<unsigned int>(&chain_length_fake)->default_value(5), "chain length for fake system: enter L+1 if you want L thermostats")
      ...
      ("cpmd_extra_compute,x", value<int>(&cpmdremote.extra_compute)->default_value(10), "compute additional (cpmd)")
...
//   box.number_of_vertices = total_gridpoints;
  ...
  cout << "Number of points discretizing the left and right z planes " << box.leftplane.size() << setw(10) << box.rightplane.size() << endl;
  ...
  cout << "Total charge inside" << box.total_charge_inside(ion) << endl;
  ...
  else
  {
    ...
    fake_bath.push_back(THERMOSTAT(0,fake_T,box.leftplane.size(),0.0,0,0));			// finally, the coding trick: dummy bath (dummy bath always has zero mass)
  }
...
/*
  ofstream induced_density_left ("outfiles/induced_density_left.dat");
  ...
  ofstream induced_density_right("outfiles/induced_density_right.dat");
